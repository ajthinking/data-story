...
it('returns a new object if no mappings are provided', () => {
    ...
    const map = {};
    expect((0, mapReplace_1.mapReplace)(original, map)).toMatchObject({});
});
it('returns a new object with the mapped properties', () => {
    const original = { a: 1 };
    const map = { x: 'a' };
    expect((0, mapReplace_1.mapReplace)(original, map)).toMatchObject({ x: 1 });
});
it('returns a new object when the mapped property is an object', () => {
    ...
    const mapped = (0, mapReplace_1.mapReplace)(original, map);
