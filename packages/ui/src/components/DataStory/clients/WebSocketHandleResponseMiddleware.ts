import { TreeMessage, TreeResponse } from '../types';
...
export const processWaitingResponse = (message: TreeResponse) => {
  const response: TreeResponse = message;
...
export async function waitForResponse(params: TreeMessage): Promise<any> {
  const { id } = params;
